...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        try {
            ...
            const boosterRoleId = '1351759870877241394';
            const hasBoosterRole = interaction.member.roles.cache.has(boosterRoleId);
            
            if (!hasBoosterRole) {
                await interaction.editReply({
                    content: '❌ Этот розыгрыш только для бустеров сервера! У вас нет роли бустера.'
                });
                return;
            }
            ...
            if (wasAdded) {
                ...
                await interaction.editReply({
                    content: `🚀 Вы успешно присоединились к розыгрышу для бустеров "${giveaway.prize}"!\n\nУчастников: ${participants.length}/${giveaway.spots}`
                });
            } else {
                await interaction.editReply({
            ...
            console.error('Ошибка при участии в розыгрыше для бустеров:', error);
            await interaction.editReply({
