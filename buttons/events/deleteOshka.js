...
module.exports = {
    data: new ButtonBuilder()
        .setCustomId('oshka-delete')
        .setLabel('❌ | Удалить шаблон')
        ...
        try {
            ...
            if (allTemplates.length === 0) {
                await interaction.editReply({
                    content: '❌ Нет доступных шаблонов Ошки для удаления.'
                });
                return;
            }
                ...
                return new ActionRowBuilder()
                    .addComponents(
                        new StringSelectMenuBuilder()
                            ...
                            .setPlaceholder(`Выберите шаблон для удаления (стр. ${page + 1}/${totalPages})`)
                            .addOptions(
            ...
            const createPaginationButtons = (page) => {
                ...
                if (page > 0) {
                    buttons.push(
                        new ButtonBuilder()
                            .setCustomId(`oshka_delete_prev_${page}`)
                ...
                buttons.push(
                    new ButtonBuilder()
                        .setCustomId('oshka_delete_page_info')
                ...
                if (page < totalPages - 1) {
                    buttons.push(
                        new ButtonBuilder()
                            .setCustomId(`oshka_delete_next_${page}`)
            ...
            // Формируем сообщение
            let content = `**Выберите шаблон Ошки для удаления:**\n`;
            ...
            const collector = message.createMessageComponentCollector({
                filter: (i) => i.user.id === interaction.user.id && 
                    (i.customId.startsWith('oshka_delete_prev_') || 
                     i.customId.startsWith('oshka_delete_next_')),
                time: 300000 // 5 минут
            });

            collector.on('collect', async (i) => {
                try {
                    if (i.customId.startsWith('oshka_delete_prev_')) {
                        currentPage = Math.max(0, currentPage - 1);
                    } else if (i.customId.startsWith('oshka_delete_next_')) {
                    ...
                    // Обновляем сообщение
                    let newContent = `**Выберите шаблон Ошки для удаления:**\n`;
