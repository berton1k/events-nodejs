...
module.exports = {
    data: new ModalBuilder()
        .setCustomId('modal-giveaway-create')
        .setTitle('Создать новый розыгрыш')
        .addComponents(
    ...
    async execute(interaction) {
        ...
        const spots = interaction.fields.getTextInputValue('giveaway_spots');
        const endDate = interaction.fields.getTextInputValue('giveaway_end_date');
        const imageUrl = interaction.fields.getTextInputValue('giveaway_image');
        ...
        try {
            ...
            if (giveawayNotificationsChannelId) {
                ...
                if (giveawayNotificationsChannel) {
                    // Отправляем уведомление о розыгрыше
                    const embed = new EmbedBuilder()
                        ...
                        .setDescription(`**Приз:** ${prize}\n` +
                            ...
                            `🎊 Розыгрыш начался! Удачи всем участникам!`)
                        .setTimestamp()
                        .setFooter({ text: `ID розыгрыша: ${giveawayId}` });
            ...
            await interaction.editReply({
                content: `Розыгрыш "${prize}" успешно создан! ID: ${giveawayId}`
            });
